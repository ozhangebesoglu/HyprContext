  ...
  if (!mounted) {
    return (
      <div className="relative inline-block">
        <div className={`relative flex ${s.container} items-center rounded-full bg-gray-200/20 p-1`} />
      </div>
    );
  }
  ...
  return (
    <div className="relative inline-block">
      ...
      <motion.button
        ...
        className={`relative flex ${s.container} items-center rounded-full p-[4px] transition-all duration-300 focus:outline-none`}
        style={{
          background: isDark
      ...
      >
        ...
        />
        {/* Background Icons */}
        <div className={`absolute inset-0 flex items-center justify-between ${s.px}`}>
          <Sun size={s.icon} className={isDark ? 'text-yellow-100' : 'text-amber-600'} />
          <Moon size={s.icon} className={isDark ? 'text-yellow-100' : 'text-slate-700'} />
        </div>

        {/* Circular Thumb with Bouncy Spring Physics */}
        <motion.div
          className={`relative z-10 flex ${s.thumb} items-center justify-center rounded-full overflow-hidden`}
          style={{
            background: isDark
        ...
        >
          ...
          <div className="relative z-10">
            {isDark ? (
              <Moon size={s.icon} className="text-yellow-200" />
            ) : (
