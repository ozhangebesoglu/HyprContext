...
interface FileTreeProps {
  ...
  onFileSelect?: (file: FileNode) => void  // File selection callback
...
function FileItem({ node, depth, isLast, parentPath, selectedFile, onFileSelect }: FileItemProps) {
...
export function FileTree({ data, className, selectedFile, onFileSelect, title = "Dosyalar" }: FileTreeProps) {
  return (
    <div
      className={cn(
        "glass glass-subtle rounded-xl p-4 font-mono",
        className,
      )}
    >
      {/* Header */}
      ...
      <div className="space-y-0.5 max-h-[400px] overflow-y-auto">
        {data.length === 0 ? (
          <p className="text-sm text-muted text-center py-4">Dosya bulunamadı</p>
        ) : (
          data.map((node, index) => (
            <FileItem 
              key={node.name} 
